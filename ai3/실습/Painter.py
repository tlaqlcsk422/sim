...
mouse_entry = Entry(info_frame,
                    textvariable = mouse_text, width=20)
mouse_entry.grid(row= 0, column= 0)

#텍스트 레이블
input_label = Label(info_frame, text="   Text input:",
                    width=15)
input_label.grid(row= 0, column= 1)

#텍스트 엔트리창
input_text = StringVar()
input_entry = Entry(info_frame, textvariable=input_text,
                    width=15)
...
def mouseLUp(event):
    ...
    #선 그리기 모드
    if draw_mode == 2:
        canvas.create_line(x1, y1, x2, y2, width=2, fill=draw_color)

    #네모 그리기 모드
    elif draw_mode ==3:
        canvas.create_rectangle(x1, y1, x2, y2, width=2, fill=draw_color)

    #원 그리기 모드
    elif draw_mode == 4:
        canvas.create_oval(x1, y1, x2, y2, outline=draw_color)
...
clear_button = Button(style_frame, text='C',width=3)
...
yellow_button.grid(row =0, column =3, sticky =E)
blue_button.grid(row =0, column =4, sticky =E)
green_button.grid(row =0, column =5, sticky =E)
